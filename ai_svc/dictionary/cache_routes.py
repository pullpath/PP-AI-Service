...
def refresh_cache_background(word: str, section: str, entry_index: Optional[int] = None,
                             sense_index: Optional[int] = None, phrase: Optional[str] = None,
    ...
    try:
        ...
        result = dictionary_service.lookup_section(
            word=word,
            section=section,
        ...
        if result.get('success'):
            use_zh_cache = cache_service._is_zh_cn(lang)
            if use_zh_cache:
                if section == 'basic':
                    cache_service.set_basic_zh(word, result)
                elif section == 'common_phrases':
                    cache_service.set_common_phrases_zh(word, result)
                elif section in ['etymology', 'word_family', 'usage_context', 'cultural_notes', 'frequency']:
                    if entry_index is not None:
                        cache_service.set_entry_section_zh(word, entry_index, section, result)
                elif section in ['detailed_sense', 'examples', 'usage_notes']:
                    if entry_index is not None and sense_index is not None:
                        cache_service.set_sense_section_zh(word, entry_index, sense_index, section, result)
            else:
                if section == 'basic':
                    cache_service.set_basic(word, result)
                elif section == 'common_phrases':
                    cache_service.set_common_phrases(word, result)
